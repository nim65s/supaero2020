...
def createUR5WithObstacles(path='/opt/openrobots/share/example-robot-data/',
                           urdf='/opt/openrobots/share/example-robot-data/ur_description/urdf/ur5_gripper.urdf',
                           initViewer=True):
    ...
    oMobs = [[0.40, 0., 0.30, np.pi / 2, 0, 0], [-0.08, -0., 0.69, np.pi / 2, 0, 0],
             [0.23, -0., 0.04, np.pi / 2, 0, 0], [-0.32, 0., -0.08, np.pi / 2, 0, 0]]
    ...
    for i, xyzrpy in enumerate(oMobs):
        ...
        obs.placement = pio.SE3(
            rotate('x', xyzrpy[3]) * rotate('y', xyzrpy[4]) * rotate('z', xyzrpy[5]),
            np.array([xyzrpy[:3]]).T)  # Set object placement wrt parent
...
class Target:
    ...
    def __init__(self, viewer, color=[.0, 1.0, 0.2, 1.0], size=0.05, position=None):
        ...
        if position is not None and viewer is not None:
            self.display()

    def display(self):
        self.viewer.gui.applyConfiguration(self.name, [self.position[0], 0, self.position[1], 1., 0., 0.0, 0.])
